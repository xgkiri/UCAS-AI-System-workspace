...
def _conv_layer(input, weights, bias):
    ...
    conv_output = tf.nn.conv2d(input, weights, strides=1, padding="SAME")
    conv_output = tf.nn.bias_add(conv_output, bias.flatten())
    return conv_output

def _pool_layer(input):
    # TODO: 定义最大池化的操作步骤，input 为输入张量，返回池化操作后的计算结果
    pool_output = tf.nn.max_pool2d(input, ksize=[1, 2, 2, 1], strides=2, padding="VALID")
    return pool_output
