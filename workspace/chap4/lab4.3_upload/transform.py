...
def net(image, type=0):
    ...
    conv1 = _conv_layer(image, 32, 9, 1, relu=True)
    conv2 = _conv_layer(conv1, 64, 3, 2, relu=True)
    conv3 = _conv_layer(conv2, 128, 3, 2, relu=True)

    res_block1 = _residual_block(conv3, 3)
    res_block2 = _residual_block(res_block1, 3)
    res_block3 = _residual_block(res_block2, 3)
    res_block4 = _residual_block(res_block3, 3)
    res_block5 = _residual_block(res_block4, 3)

    tran_conv1 = _conv_tranpose_layer(res_block5, 64, 3, 2)
    tran_conv2 = _conv_tranpose_layer(tran_conv1, 32, 3, 2)
    tran_conv3 = _conv_tranpose_layer(tran_conv2, 3, 9, 1)
    # tran_conv3 = _conv_layer(tran_conv2, 3, 9, 1, relu=True)

    #TODO：最后一个卷积层的输出再经过 tanh 函数处理，最后的输出张量 preds 像素值需限定在 [0,255] 范围内
    preds = tf.nn.tanh(tran_conv3) * 255
    return preds
...
def _conv_tranpose_layer(net, num_filters, filter_size, strides, type=0):
    ...
    tf_shape = tf.stack(new_shape) # 从0维的向量转化成1维的张量

    strides_shape = [1, strides, strides, 1]

    # TODO：进行转置卷积计算
    net = tf.nn.conv2d_transpose(net, weights_init, tf_shape, strides=strides_shape, padding="SAME")
...
def _residual_block(net, filter_size=3, type=0):
    ...
    temp1 = _conv_layer(net, out_channels, filter_size, strides=1, relu=True)
    temp2 = _conv_layer(temp1, out_channels, filter_size, strides=1, relu=False)
    net = temp2 + net
    net = tf.nn.relu(net)
